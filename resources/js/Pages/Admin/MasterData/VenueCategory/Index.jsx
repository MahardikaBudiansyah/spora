...
export default function Index() {
    const { venue_categories = [] } = usePage().props;

    const [rows, setRows] = useState(venue_categories.data || []);
    ...
    const {
        ...
        endpoint: (id) =>
            route("admin.master-data.venue-categories.inline-update", { id }),
        debounce: 400,
    });
    ...
    const handleDelete = (row) => {
        setSelectedVenueCategory(row);
        setShowModal(true);
    };

    const deleteVenueCategory = () => {
        if (!selectedVenueCategory) return;

        router.delete(
            route("admin.master-data.venue-categories.destroy", {
                id: selectedVenueCategory.id,
            }),
            {
                preserveScroll: true,
                onSuccess: () => {
                    ...
                    setRows((prev) =>
                        prev.filter((r) => r.id !== selectedVenueCategory.id),
                    );

                    setSelectedVenueCategory(null);
                },
            },
        );
    };
    ...
    const saveNewRow = async (row) => {
        ...
        try {
            const response = await axios.post(
                route("admin.master-data.venue-categories.store"),
    ...
    const handleNewRowChange = (rowId, venue, value) => {
        setRows((prev) => {
            const updated = prev.map((r) =>
                r.id === rowId ? { ...r, [venue]: value } : r,
            );
    ...
    const columns = [
        {
            ...
            render: (_, __, index) => {
                const currentPage =
                    venue_categories.current_page ||
                    venue_categories.meta?.current_page ||
                    1;
                const perPage =
                    venue_categories.per_page ||
                    venue_categories.meta?.per_page ||
                    10;
    ...
    return (
        <AdminLayout>
            <Head title="Kelola Kategori Venue" />
            ...
            <Card className="flex flex-col h-full rounded-md shadow-none">
                        ...
                        <Table
                                ...
                                if (col.editable) {
                                    ...
                                    return (
                                        <InlineEditingCell
                                            ...
                                            focusedField={focusedVenue}
                                            setFocusedField={setFocusedVenue}
                            ...
                            emptyState={
                                <div className="text-center text-sm text-gray-600 dark:text-gray-400">
                                    Tidak ada data Kategori Venue.
                                </div>
                            }
                        />

                        <Pagination
                            links={venue_categories.links}
                            meta={venue_categories}
                            className="p-6 my-2"
                        />
                    </div>
                </CardBody>
                ...
                {showModal && selectedVenueCategory && (
                    <DeleteModal
                        show={showModal}
                        onClose={() => setShowModal(false)}
                        onDelete={deleteVenueCategory}
                        title="Hapus Data Kategori Venue"
                        description={`Yakin ingin menghapus data kategori "${selectedVenueCategory.name}"?`}
                    />
                )}
            </Card>
        </AdminLayout>
