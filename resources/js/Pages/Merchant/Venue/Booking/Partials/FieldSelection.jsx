...
export default function FieldSelection({
    ...
    const selectedDateFormatted = useMemo(
        () =>
            selectedDate
                ? formatWithPattern(selectedDate, "cccc, d LLL yyyy")
                : "-",
        [selectedDate]
    );
    ...
    const tableData = useMemo(() => {
        ...
        return timeslots.map((slot) => {
            ...
            return {
                ...
                timeSlots: slot.name,
                price: slot.price,
                status: isSelected ? "Dipilih" : slot.status_label,
    ...
    return (
        <div className="flex flex-col w-full text-left">
            ...
            <div className="py-2 flex flex-col gap-2">
                <div className="md:w-1/4 flex flex-row md:justify-between gap-2 items-center">
                    <span className="font-bold">Pilih Jam Main</span>
                    <span className="ml-2">:</span>
                </div>
                <div className="flex-1 max-h-96 w-full overflow-y-auto custom-scrollbar">
                    <Table
                        columns={columns}
                        data={tableData}
                        renderCell={(col, row) => {
                            if (col.key === "checkbox") {
                                const isAvailable =
                                    row.status === "Tersedia" ||
                                    row.status === "Dipilih";
                                const isChecked = row.status === "Dipilih";
                                return (
                                    <Checkbox
                                        checked={isChecked}
                        ...
                        getTrProps={(row) => {
                            ...
                            const classes = {
                                ...
                                Dipilih:
                                    "bg-primary-200 dark:bg-primary-700 border-primary-300 dark:border-primary-800 hover:bg-primary-300 dark:hover:bg-primary-400 font-semibold",
                                Dipesan:
